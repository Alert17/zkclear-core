...
impl Storage for RocksDBStorage {
    fn save_block(&self, block: &Block) -> Result<(), StorageError> {
        ...
        let metadata_cf = self.db.cf_handle(CF_METADATA)
            .ok_or_else(|| StorageError::DatabaseError("CF_METADATA not found".to_string()))?;
        
        self.db.put_cf(metadata_cf, b"latest_block_id", Self::encode_block_id(block.id))
            .map_err(|e| StorageError::DatabaseError(e.to_string()))?;
    ...
    fn get_latest_block_id(&self) -> Result<Option<BlockId>, StorageError> {
        ...
        match self.db.get_cf(cf, b"latest_block_id")
            .map_err(|e| StorageError::DatabaseError(e.to_string()))? {
    ...
    fn get_transactions_by_block(&self, block_id: BlockId) -> Result<Vec<Tx>, StorageError> {
        ...
        let mut txs = Vec::new();
        let mut iter = self.db.iterator_cf(cf, rocksdb::IteratorMode::From(&prefix, rocksdb::Direction::Forward));
    ...
    fn save_state_snapshot(&self, state: &State, block_id: BlockId) -> Result<(), StorageError> {
        let cf = self.db.cf_handle(CF_STATE_SNAPSHOTS)
            .ok_or_else(|| StorageError::DatabaseError("CF_STATE_SNAPSHOTS not found".to_string()))?;
        ...
        self.db.put_cf(metadata_cf, b"latest_state_snapshot_block_id", Self::encode_block_id(block_id))
            .map_err(|e| StorageError::DatabaseError(e.to_string()))?;
    ...
    fn get_latest_state_snapshot(&self) -> Result<Option<(State, BlockId)>, StorageError> {
        ...
        let snapshot_block_id = match self.db.get_cf(metadata_cf, b"latest_state_snapshot_block_id")
            .map_err(|e| StorageError::DatabaseError(e.to_string()))? {
