...
impl DeserializedStarkProof {
    ...
    pub fn from_bytes(
        ...
        let proof = Proof::from_bytes(proof_bytes).map_err(|e| {
        ...
        let public_inputs =
            Self::extract_public_inputs_from_proof(&proof, proof_bytes, expected_public_inputs);

        // Extract commitments from proof structure
        // Winterfell proof contains commitments that we can extract
        let (trace_commitment, constraint_commitment) =
            Self::extract_commitments_from_proof(&proof, proof_bytes);
    ...
    fn extract_commitments_from_proof(
        proof: &Proof,
        proof_bytes: &[u8],
    ...
    pub fn verify_commitments(&self) -> bool {
        let trace_valid = self
            .trace_commitment
            .map(|c| c.iter().any(|&b| b != 0))
            .unwrap_or(false);

        let constraint_valid = self
            .constraint_commitment
            .map(|c| c.iter().any(|&b| b != 0))
            .unwrap_or(false);
    ...
    pub fn verify_structure(&self) -> bool {
        ...
        let state_roots_different =
            self.public_inputs.prev_state_root != self.public_inputs.new_state_root;
    ...
    pub fn summary(&self) -> String {
        format!(
            ...
            self.proof_length,
            self.num_queries,
            if self.trace_commitment.is_some() {
                "present"
            } else {
                "missing"
            },
            if self.constraint_commitment.is_some() {
                "present"
            } else {
                "missing"
            }
